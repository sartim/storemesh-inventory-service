...
func TestInventoryReservationLifecycle(t *testing.T) {
	...
	if err != nil || reserved.GetStock().GetAvailable() != 3 { t.Fatalf("reserve: stock=%v err=%v", reserved, err) }
	...
	if err != nil || released.GetStock().GetAvailable() != 5 { t.Fatalf("release: stock=%v err=%v", released, err) }
}
...
func TestInventoryRejectsOversell(t *testing.T) {
	...
	if status.Code(err) != codes.FailedPrecondition { t.Fatalf("expected failed precondition, got %v", err) }
}
